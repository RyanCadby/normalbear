...
export const serve = done => {
    server.init({
        proxy: "http://localhost/normalbear/",
    });
    done();
};
...
export const styles = () => {
    return src(['src/scss/**/*.scss'])
        ...
        .pipe(gulpif(!PRODUCTION,replace(/http:\/\/localhost:3000\/normalbear\//g, 'http://localhost/normalbear/')))
        .pipe(gulpif(PRODUCTION, replace(/http:\/\/localhost:3000\/normalbear\//g, '/')))
        .pipe(gulpif(PRODUCTION, replace(/http:\/\/localhost\/normalbear\//g, '/')))
        .pipe(gulpif(PRODUCTION, replace(/\/\/localhost\/normalbear\//g, '/')))
        .pipe(gulpif(PRODUCTION, replace(/localhost\/normalbear\//g, '/')))
